    ...
    setTimeout(async () => {
        if (targetNumber > currentBlockNumber) {
            ...
            try {
                ...
                if(collection.last.block.number - collection.first.block.number + 1 <= NOTIFICATION_SIZE) {
                    for (let bn = collection.first.block.number; bn <= collection.first.block.number; bn++) {
...
async function buildTask(bn: number): Promise<BlockTask> {
    ...
    const [blockFee, events, validators, sessionInfo] = await Promise.all([
